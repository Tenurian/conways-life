...
let generateNeighbors = (y, x, dist = 1) => {
	...
	if(y < rows && y > 0 && x < cols && x > 0 && dist > 0.0625)
	{
		let directions = [
			"North",
			"North-East",
...
let generateHash = () => {
	...
	while(zeroRX.test(strip)){
		...
		try {
			...
			//console.log({test: zeroRX.test(strip), start, leng, mat, comp, strip});
		} catch (e) {
...
let loadHash = (hash) => {
	...
	while(zeroGenRX.test(strip)){
		...
		console.log({mat, fil, decomp, strip});
	}
